export const cortexPaperSections = [
  ...
  },
  {
    ...
    "blocks": [
      ...
      "acknowledge that achieving practical efficiency at scale will present significant technical challenges; addressing these is a clear near-term priority. These controls postpone an unhealthy token-to-parameter ratio, yet they cannot erase it once personalized medicine enters full flow. With a realistic DNA tokenizer (128- to 512-base-pair (bp) windows), a 3 Gbp human genome yields ≈ 6 to 24 million tokens. Adding methylation, RNA-seq, longitudinal labs, imaging metadata, and wearable telemetry lifts a deeply profiled patient into the 0.1 to 0.5 billion-token range.",
      "● Base pre-training. A future 30 B-parameter Nicole will require 300 to 600 billion tokens during its one-time foundation model pre-train (10 to 20 tokens ⁄ parameter). That volume will come from global literature, public omics archives, and simulated data, and will run on data center GPUs. ● Incremental fine-tuning. After deployment, each gyrus will ingest ≈ 10 to 20 million new tokens per day from fresh papers, EHR deltas, and lab feeds. Because these daily slices are < 0.01 % of the base corpus, the parameter-to-token heuristic no longer governs capacity; runtime and VRAM budget dominate.",
  ...
  },
  {
    ...
    "blocks": [
      ...
      "● Single-node optimisation: squeeze every joule from the first 512 GB Studio with fused kernels, mixed-precision accumulators, and I/O pre-fetch. ● Collective-layer evaluation: run the same fine-tune under MLX, exo, and a fallback gRPC sharder to see which stack holds at least 70% of theoretical FLOPs across a three-node mesh.",
  ...
  },
  {
    ...
    "blocks": [
      ...
      "³ Fedus, William, Barret Zoph, and Noam Shazeer. “Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity.” Journal of Machine Learning Research 23, no. 120 (2022): 1 to 39.",
